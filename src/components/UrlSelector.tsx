...
export function UrlSelector({ value, customUrl, onChange }: UrlSelectorProps) {
  // 综合外部传入与当前值的自定义 URL
  const activeCustomUrl = customUrl || (!(PRESET_URLS as readonly string[]).includes(value) ? value : undefined);
  const showCustomOption = activeCustomUrl && !(PRESET_URLS as readonly string[]).includes(activeCustomUrl);
      ...
      {showCustomOption && (
        ...
        >
          <span
            className={cn(
              'w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center',
              value === activeCustomUrl ? 'border-yellow-500' : 'border-gray-600'
            )}
          >
            {value === activeCustomUrl && <span className="w-2 h-2 rounded-full bg-yellow-500" />}
          </span>
          <span className="text-sm font-mono truncate flex-1">{activeCustomUrl}</span>
          <span className="text-xs text-yellow-500/80 flex-shrink-0">（当前自定义）</span>
        </button>
      )}
    </div>
  );
